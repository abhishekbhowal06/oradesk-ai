...
async function testKnowledgeBase() {
    ...
    try {
        const doc = await knowledgeService.addDocument(clinicId, testContent, { source: 'test-script' });
        if (!doc) throw new Error('Document creation returned null');
        console.log('✅ Document added successfully');
    } catch (err) {
        console.error('❌ Failed to add document:', err);
        // Proceed to search anyway (might find older docs) but logging error
    }
