...
async function runEndToEndTest() {
  return new Promise((resolve, reject) => {
    ...
    ws.on('open', () => {
      ...
      console.log(
        `   Target: <100ms | ${testResults.websocketConnection < 100 ? 'PASS' : 'FAIL'}\n`,
      );
      ...
      setTimeout(() => {
        ...
        testResults.streamInitialization = latency.measure(
          'stream_init_start',
        ...
        console.log(
          `   Target: <300ms | ${testResults.streamInitialization < 300 ? 'PASS' : 'FAIL'}\n`,
        );
...
runEndToEndTest()
  .then((results) => {
    ...
    console.log(
      `  WebSocket Connection:    ${results.websocketConnection}ms ${results.websocketConnection < 100 ? '✅' : '❌'}`,
    );
    console.log(
      `  Stream Initialization:   ${results.streamInitialization}ms ${results.streamInitialization < 300 ? '✅' : '❌'}`,
    );
    console.log(
      `  Audio Transmission:      ${results.audioFrameTransmission}ms ${results.audioFrameTransmission < 500 ? '✅' : '❌'}`,
    );
    ...
    console.log(
      `  Status: ${totalLatency < 500 ? '✅ READY FOR REAL-TIME' : '⚠️ NEEDS OPTIMIZATION'}\n`,
    );
    ...
    expectedLatency.total =
      expectedLatency.sttPartial +
      expectedLatency.earlyExit +
      expectedLatency.turnGap +
      expectedLatency.ttsFirstChunk;
    ...
    console.log(
      `  📊 IMPROVEMENT: ${(((10000 - expectedLatency.total) / 10000) * 100).toFixed(1)}% vs old system\n`,
    );
