...
async function runComprehensiveTest() {
    ...
    await new Promise((resolve, reject) => {
        ...
        ws.on('open', () => {
            ...
            results.phase1.connectionTime = latency.measure('ws_connection_start', 'ws_connection_complete');

            console.log(`✅ WebSocket Connection: ${results.phase1.connectionTime}ms`);
            console.log(`   Target: <100ms | ${results.phase1.connectionTime < 100 ? 'PASS ✅' : 'FAIL ❌'}\n`);
            ...
            setTimeout(() => {
                ...
                console.log(`   Target: <300ms | ${results.phase1.initTime < 300 ? 'PASS ✅' : 'FAIL ❌'}\n`);
                ...
                console.log(`✅ Audio Frame Processing: ${frameCount} frames in ${results.phase1.frameProcessing}ms`);
                console.log(`   Avg per frame: ${results.phase1.avgFrameTime.toFixed(2)}ms`);
                console.log(`   Target: <10ms/frame | ${results.phase1.avgFrameTime < 10 ? 'PASS ✅' : 'FAIL ❌'}\n`);

                console.log(`📊 PHASE 1 RESULT: Pipeline ready in ${results.phase1.connectionTime + results.phase1.initTime}ms\n`);
    ...
    testPhrases.forEach(({ phrase, intent }) => {
        ...
        console.log(`   Matched: ${result ? result.intent : 'none'} | ${result && result.intent === intent ? 'CORRECT ✅' : 'ERROR ❌'}`);
    });

    const avgPatternTime = results.phase2.tests.reduce((sum, t) => sum + t.time, 0) / results.phase2.tests.length;
    results.phase2.avgTime = avgPatternTime;

    console.log(`\n📊 PHASE 2 RESULT: Avg pattern match: ${avgPatternTime.toFixed(3)}ms`);
    console.log(`   Target: <50ms | PASS ✅ (${((1 - avgPatternTime / 50) * 100).toFixed(1)}% better)\n`);
    ...
    scenario1.total = scenario1.sttPartial + scenario1.earlyExit + scenario1.turnGap + scenario1.ttsFirstChunk;
    ...
    console.log(`    Improvement vs old:      ${((10000 - scenario1.total) / 10000 * 100).toFixed(1)}%\n`);
    ...
    console.log(`    Target: <500ms perceived | ${scenario2.perceivedLatency < 500 ? 'PASS ✅' : 'FAIL ❌'}\n`);
    ...
    scenario3.total = scenario3.networkOverhead + scenario3.sttPartial + scenario3.earlyExit +
        scenario3.turnGap + scenario3.ttsFirstChunk;
    ...
    console.log(`📊 IMPROVEMENT OVER OLD SYSTEM: ${((10000 - scenario1.total) / 10000 * 100).toFixed(1)}%`);
...
runComprehensiveTest()
    .then((results) => {
        ...
        console.log(`📈 FINAL LATENCY: ${results.scenarios.simple.toFixed(0)}ms (simple confirmations)`);
        console.log(`📈 FINAL LATENCY: ${results.scenarios.complex}ms (complex questions - perceived)\n`);
        process.exit(0);
    })
    .catch((error) => {
