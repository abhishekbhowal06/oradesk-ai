...
export async function validateCallContext(context: CallContext): Promise<void> {
    ...
    if (!quotaCheck.allowed) {
        ...
        throw new CallServiceError(
            ...
            CallErrorCodes.QUOTA_EXCEEDED,
            429,
            { current_usage: quotaCheck.currentUsage, limit: quotaCheck.limit }
        );
    }
...
export async function initiateTwilioCall(
    ...
    try {
        ...
        await supabase
            ...
            .update({
                status: 'failed',
                escalation_reason: error instanceof CircuitOpenError
