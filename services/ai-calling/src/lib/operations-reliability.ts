...
export async function generateClinicHealth(clinicId: string): Promise<ClinicHealthReport> {
    ...
    if (recentCalls && recentCalls.length > 0) {
        ...
        // 2. Unreachable Rate
        const unreachable = recentCalls.filter(c =>
            c.status === 'no_answer' || c.outcome === 'unreachable'
        ).length;
    ...
    signals.push({
        ...
        status: (escalationBacklog || 0) === 0 ? 'healthy'
            : (escalationBacklog || 0) <= 3 ? 'warning' : 'critical',
        humanMessage: (escalationBacklog || 0) === 0
    ...
    if (weekCalls && weekCalls.length > 0) {
        ...
        signals.push({
            ...
            status: effectivenessRate >= 60 ? 'healthy'
                : effectivenessRate >= 40 ? 'warning' : 'critical',
            humanMessage: effectivenessRate >= 60
                ? 'AI is confirming appointments effectively'
                : 'AI confirmation rate is low',
            actionNeeded: effectivenessRate < 40
                ? 'Review AI settings or consider manual confirmation' : undefined
        });
    }
    ...
    const overallHealth = criticalCount > 0 ? 'critical'
        : warningCount > 2 ? 'needs_attention' : 'healthy';
