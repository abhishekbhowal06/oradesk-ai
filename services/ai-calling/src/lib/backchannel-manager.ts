...
export class BackchannelManager {
   ...
   */
  start(onSpeak: (text: string) => Promise<void>, onTimeout: () => Promise<void>): void {
 ...
 */
export function selectContextualBackchannel(
  context: 'greeting' | 'thinking' | 'checking' | 'waiting',
): string {
...
// Singleton instance removed to prevent concurrency issues
// Each call must instantiate its own BackchannelManager
