...
const EARLY_EXIT_PATTERNS = {
  ...
  // Denial patterns
  deny: {
    patterns: [/^(no|nope|nah|not really|i can't|cannot|won't be able)/i],
    response: 'I understand. Let me connect you with our team to reschedule.',
  },

  // Cancellation patterns
  cancel: {
    patterns: [/^(cancel|don't want|not interested|never mind|forget it)/i],
    response: "I've noted your cancellation request. A staff member will follow up with you.",
  },

  // Questions (require human)
  question: {
    patterns: [/^(what|when|where|how|why|who|can you|could you|will|is it)/i],
...
const BACKCHANNEL_LIBRARY = {
  acknowledgement: ['mm-hmm', 'okay', 'I see', 'got it', 'alright'],
  thinking: ['let me check that', 'one moment', 'just a second', 'let me pull that up'],
  clarifying: [
    'sorry?',
...
export class LatencyMonitor {
  ...
  mark(event: string): void {
    this.metrics.set(event, performance.now());
  }
  ...
  /**
   * CRASH TEST: Per-turn structured latency telemetry.
   *
   * Emits a structured JSON log for every conversation turn with:
   *   t0 = utterance_end (patient stopped speaking)
   *   t1 = gemini_start (LLM request)
   *   t2 = tool_start (if applicable)
   *   t3 = tool_end (database query time)
   *   t4 = tts_start (TTS request)
   *   t5 = tts_first_byte (audio ready to play)
   *
   * WARNING threshold: total_turnaround > 1000ms.
   */
  emitTurnMetrics(callId: string, turnIndex: number): TurnLatencyMetrics {
    const totalTurnaround = this.measure('patient_stopped', 'tts_first_chunk');
    const sttLatency = this.measure('patient_stopped', 'stt_final');
    const geminiLatency = this.measure('ai_start', 'ai_complete');
    const dbLatency = this.measure('tool_start', 'tool_complete');
    const ttsLatency = this.measure('tts_start', 'tts_first_chunk');

    const turnMetrics: TurnLatencyMetrics = {
      ...
      stt_latency: sttLatency, // t0 → stt_final
      gemini_latency: geminiLatency, // t1 → ai_complete
      db_latency: dbLatency, // t2 → t3 (only if tool used)
      tts_latency: ttsLatency, // t4 → t5

      // ── End-to-End ──
      total_turnaround: totalTurnaround, // t0 → t5

      // ── Thresholds ──
      targets: {
        total: totalTurnaround <= 500 ? '✅ <500ms' : totalTurnaround <= 1000 ? '⚠️ <1s' : '🔴 >1s',
        tts: ttsLatency <= 200 ? '✅ <200ms' : ttsLatency <= 500 ? '⚠️ <500ms' : '🔴 >500ms',
        gemini: geminiLatency <= 500 ? '✅ <500ms' : geminiLatency <= 1000 ? '⚠️ <1s' : '🔴 >1s',
      },
    };

    // Structured log for monitoring systems (Datadog, CloudWatch, etc.)
    logger.info('latency_metrics', turnMetrics);

    // ═══ ALERT: warn on slow turns ═══
    if (totalTurnaround > 1000) {
      logger.warn('🔴 SLOW TURN DETECTED — exceeds 1000ms target', {
        callId,
        turn: turnIndex,
        total_turnaround: `${totalTurnaround}ms`,
        breakdown: {
          stt: `${sttLatency}ms`,
          gemini: `${geminiLatency}ms`,
  ...
  logMetrics(callId: string): void {
    ...
    const perceivedLatency =
      this.measure('patient_stopped', 'tts_playback_started') || metrics.totalPerceived;
    ...
    logger.info(
      `  STATUS:               ${meetsTarget ? '✅ TARGET MET' : '⚠️ NEEDS OPTIMIZATION'}`,
    );
    logger.info('='.repeat(60));
