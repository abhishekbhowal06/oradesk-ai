...
const PII_PATTERNS = [
  ...
  {
    pattern:
      /\b\d{1,5}\s+[A-Za-z]+\s+(street|st|avenue|ave|road|rd|drive|dr|lane|ln|court|ct|boulevard|blvd)\b/gi,
    replacement: '[ADDRESS]',
    name: 'address',
  },
...
export function redactForLogging(text: string): string {
  ...
  // Redact potential names using a bounded, non-backtracking approach
  // Look for capitalized words (2-15 chars) that aren't at the start of a sentence.
  // Run TWICE: the first pass catches "John", the second pass catches "Smith"
  // since "[NAME]" now provides the preceding non-.?! character context.
  const nameRegex = /(?:[^.?!]\s+)\b([A-Z][a-z]{1,14})\b/g;
  logSafe = logSafe.replace(nameRegex, (match, p1) => match.replace(p1, '[NAME]'));
  logSafe = logSafe.replace(nameRegex, (match, p1) => match.replace(p1, '[NAME]'));
