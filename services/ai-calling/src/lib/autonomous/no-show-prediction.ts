...
export class NoShowPredictionEngine {
  ...
  private async assessNoShowRisk(appointment: any): Promise<AppointmentRiskAssessment> {
    ...
    if (!profile) {
      ...
      if (
        !profile.avg_confirmation_response_time_hours ||
        profile.avg_confirmation_response_time_hours > 24
      ) {
  ...
  private async logPrediction(risk: AppointmentRiskAssessment) {
    ...
    await supabase.from('no_show_predictions').insert({
      ...
      days_before_appointment: Math.floor(
        (risk.scheduledAt.getTime() - Date.now()) / (1000 * 60 * 60 * 24),
      ),
    });
  }
  ...
  async getAccuracyMetrics(clinicId: string, days: number = 30) {
    ...
    const truePositives = predictions.filter(
      (p) => p.predicted_probability >= 60 && p.actual_outcome === 'no_show',
    ).length;

    const precision =
      predictedNoShows.length > 0 ? (truePositives / predictedNoShows.length) * 100 : 0;
