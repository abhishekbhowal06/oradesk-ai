...
export class ReputationShieldEngine {
  ...
  private calculateAngerScore(transcript: string, voiceMetrics: any): number {
    ...
    if (voiceMetrics.pitch && voiceMetrics.pitch > 250) {
      // Higher pitch = stress
      score += 10;
    }

    if (voiceMetrics.speakingRate && voiceMetrics.speakingRate > 200) {
      // Fast talking = anger
      score += 10;
    }
  ...
  private determineCrisisType(
    transcript: string,
    negativeKeywords: string[],
  ): CrisisDetection['crisisType'] {
    const transcriptLower = transcript.toLowerCase();

    if (
      transcriptLower.includes('billing') ||
      transcriptLower.includes('charge') ||
      transcriptLower.includes('payment')
    ) {
      return 'billing_dispute';
    }

    if (
      transcriptLower.includes('waiting') ||
      transcriptLower.includes('hold') ||
      transcriptLower.includes('late')
    ) {
      return 'long_wait';
    }

    if (
      transcriptLower.includes('pain') ||
      transcriptLower.includes('emergency') ||
      transcriptLower.includes('hurts')
    ) {
  ...
  private calculateReviewRisk(angerScore: number, threatToLeave: boolean, patient: any): number {
    ...
    if (
      patient.created_at &&
      new Date(patient.created_at) < new Date(Date.now() - 365 * 24 * 60 * 60 * 1000)
    ) {
  ...
  async resolveCrisis(
    eventId: string,
    outcome: 'patient_satisfied' | 'partial_resolution' | 'patient_left' | 'review_posted',
  ) {
  ...
  async getMetrics(clinicId: string, days: number = 30) {
    ...
    const preventionRate =
      resolvedEvents.length > 0 ? (preventedReviews / resolvedEvents.length) * 100 : 0;
