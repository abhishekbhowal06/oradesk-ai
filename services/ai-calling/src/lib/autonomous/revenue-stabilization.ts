...
export class RevenueStabilizationEngine {
   ...
   * Detect revenue gaps
   */
  private async detectRevenueGap(
    clinicId: string,
    targets: RevenueTarget,
  ): Promise<RevenueGap | null> {
    const now = new Date();
  ...
  private async fillRevenueGap(gap: RevenueGap) {
    ...
    if (gap.gap > gap.target * 0.2) {
      // If >20% under, use discounts
  ...
  private async fillOpenSlotsWithRecall(gap: RevenueGap) {
    ...
    for (const slot of openSlots.slice(0, 5)) {
      // Fill up to 5 slots
