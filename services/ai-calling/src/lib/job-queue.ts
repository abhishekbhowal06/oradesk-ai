...
export const JOB_SCHEDULES: Record<JobType, string> = {
 ...
 */
export async function initJobQueue(): Promise<any> {
    if (bossInstance) {
        return bossInstance;
    }
 ...
 */
export function getJobQueue(): any {
    if (!bossInstance) {
...
export async function scheduleRecurringJob(
    ...
    await boss.schedule(jobType, cron, {
...
export async function registerJobHandler(
    ...
    await boss.work(jobType, async (job: Job<JobPayload>) => {
