...
import { registerJobHandler, scheduleRecurringJob, JobPayload, Job } from './job-queue';
...
import { registerCampaignWorker } from '../services/CampaignWorker';
...
export async function initJobHandlers(): Promise<void> {
    ...
    for (const clinic of clinics) {
      try {
        ...
        if (clinicHour >= 7 && clinicHour < 8) {
          ...
          // Delivery: Reports are persisted to the dashboard for staff to view on login.
          // Future: Add email/SMS delivery via notification service.
          logger.info(`Morning Huddle delivered to dashboard for ${clinic.name}`);
        }
      } catch (error) {
  ...
  await registerJobHandler('operations_health_check', async (job: Job<JobPayload>) => {
    ...
    if (failures.length > 0) {
      ...
      logger.info(
        `Auto-recovery: ${recoveries.filter((r) => r.result === 'success').length} successful`,
      );
    }
  });
  ...
  await registerJobHandler('recall_campaign_batch', async (job: Job<JobPayload>) => {
    ...
    // Recall campaign processing is handled by the Recall Engine.
    // This job is a placeholder for batch queue dispatch when the recall
    // engine scales beyond single-cycle processing.
    logger.info('Recall campaign batch: no pending candidates');
  });

  // 7. OUTBOUND CAMPAIGN CALLS
