...
import { alertEngineers } from './alerting';
...
export class CircuitBreaker {
  ...
  private recordFailure(): void {
    ...
    if (this.state === 'half-open') {
      ...
      this.triggerAlert('Opened after half-open failure');
    } else if (this.failureCount >= this.config.failureThreshold) {
      this.state = 'open';
      logger.warn(`Circuit ${this.config.name} opened after ${this.failureCount} failures`);
      this.triggerAlert(`Opened after ${this.failureCount} sequential failures`);
    }
  }

  /**
   * Internal helper to dispatch PagerDuty alerts
   */
  private triggerAlert(reason: string): void {
    alertEngineers({
      serviceName: this.config.name,
      description: `Circuit Breaker tripped OPEN: ${reason}`,
      severity: 'critical',
      details: {
      ...
      logger.error('Failed to trigger circuit breaker alert', { error: err.message });
    });
  }
