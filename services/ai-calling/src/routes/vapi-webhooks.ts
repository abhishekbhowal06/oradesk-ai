...
router.post('/vapi/events', async (req: Request, res: Response) => {
  ...
  // Synthesize a unique event ID for Vapi deduplication
  const eventId = (req.headers['x-vapi-event-id'] as string) || `${event.call?.id}_${event.type}`;

  const isNew = await checkAndLockWebhook('vapi', eventId);
  if (!isNew) {
    res.status(200).send('duplicate_event_ignored');
    return;
  }
  ...
  try {
    switch (event.type) {
      ...
      case 'function-call':
        ...
        // NOTE: Booking logic is handled by the real-time stream handler's tool loop.
        // This webhook path is for async Vapi integrations (non-streaming).
        // If needed, dispatch to the appropriate tool handler here.
        break;
