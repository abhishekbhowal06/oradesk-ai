...
const errorContent: Record<ErrorType, { icon: ReactNode; title: string; description: string }> = {
  ...
  server: {
    ...
    description:
      'Our systems are experiencing high demand. This is typically resolved within minutes.',
  },
  permission: {
    icon: <ShieldAlert className="h-6 w-6" />,
    title: 'Access Restricted',
    description:
      'You do not have permission to view this resource. Contact your administrator if needed.',
  },
  validation: {
    icon: <AlertCircle className="h-6 w-6" />,
    title: 'Invalid Request',
    description:
      'The submitted data could not be processed. Please review your input and try again.',
  },
  generic: {
    icon: <AlertCircle className="h-6 w-6" />,
...
export function ErrorState({
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg font-medium text-foreground mb-2">{title || content.title}</h3>
      <p className="text-sm text-muted-foreground max-w-sm">{description || content.description}</p>
      {onRetry && (
        <button
          onClick={onRetry}
          className={cn(
